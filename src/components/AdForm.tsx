...
export default function AdForm(props: AdFormProps) {
  ...
  const {data: categoriesData, error: categoriesError, loading: categoriesLoading} = useQuery<{items: CategoryType[] }>(queryAllCategories);
  const categories = categoriesData ? categoriesData.items : [];
  const router = useRouter();
  const [doCreate, { loading: createLoading }] = useMutation(mutationCreateAd, {refetchQueries: [queryAllAds],});
  const [doUpdate, { loading: updateLoading }] = useMutation(mutationUpdateAd, {refetchQueries: [queryAd, queryAllAds],});
  const loading = createLoading || updateLoading;

  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError(undefined);
    const data: AdFormData = {title, description, imgUrl, price, category: categoryId ? { id: Number(categoryId) } : null, tags: [],};
    ...
    if (data.title.trim().length < 3) {
      ...
      if (!props.ad) {
        const result = await doCreate({variables: {data: data,},
        });
        ...
        const result = await doUpdate({
          variables: {id: props.ad?.id, data: data,},
        });
  ...
  return (
    <Layout title="Nouvelle offre">
      <main className="main-content">
        ...
        {error === "title" && (<p>Titre manquant</p>)}
